...
//WIFI, PASSWORD
const char* ssid = "Megacable_86A0";
const char* password = "6578311372";
int state; 
int ledpin = 12; // Pin que usaremos para encender el led 
...
void setup() {
  ...
  if (!MDNS.begin("Arduino1")) {             // Asignamos la DNS " Arduino2" al Esp8266 para que la aplicación web pueda comunicarse con él a pesar de que cambie la ip.
 ...
 if (!MDNS.begin("Arduino1")) {             // Start the mDNS responder for esp8266.local
    Serial.println("Error setting up MDNS responder!");
 }
 Serial.println("mDNS responder started");
